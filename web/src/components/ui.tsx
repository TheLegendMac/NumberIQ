import { useState, type ReactNode, type SelectHTMLAttributes, type ButtonHTMLAttributes, type InputHTMLAttributes } from 'react';
...
export function Button({ variant = 'default', size, className = '', ...rest }: ButtonHTMLAttributes<HTMLButtonElement> & {
  variant?: 'default' | 'primary' | 'ghost' | 'danger'; size?: 'sm' | 'lg';
}) {
  const v = variant === 'default' ? '' : `btn-${variant}`;
  return <button className={`btn ${v} ${size ? `btn-${size}` : ''} ${className}`} {...rest} />;
}
...
export function Field({ label, children, hint }: { label: string; children: ReactNode; hint?: string }) {
  return (
    <div className="field">
      ...
      {children}
      {hint && <span className="inline-note">{hint}</span>}
    </div>
  );
}
...
export function Meter({ value, tone }: { value: number; tone?: string }) {
  return (
    <div className="meter" role="presentation">
      <div className="meter-fill" style={{ width: `${Math.max(0, Math.min(100, value))}%`, background: tone }} />
    </div>
  );
}
